...
$(function () {
    ...
    ***************************/
    function setupLogoScheme() {
        if (window.milLogoSchemeHandler) {
            window.removeEventListener('scroll', window.milLogoSchemeHandler);
            window.removeEventListener('resize', window.milLogoSchemeHandler);
            window.milLogoSchemeHandler = null;
        }

        const headerFrame = document.querySelector('.mil-frame');
        if (!headerFrame) {
            return;
        }
        const headerLogo = headerFrame.querySelector('.mil-logo');
        if (!headerLogo) {
            return;
        }
        const darkSections = Array.from(document.querySelectorAll('.mil-dark-bg'));

        const updateScheme = () => {
            const logoRect = headerLogo.getBoundingClientRect();
            const checkPoint = logoRect.top + logoRect.height / 2;
            let onDark = false;
            ...
            headerFrame.classList.toggle('mil-on-dark', onDark);
        };

        window.milLogoSchemeHandler = updateScheme;

        window.addEventListener('scroll', updateScheme);
